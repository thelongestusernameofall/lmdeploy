...
loras="hql-davinci-004=/data2/simon/models/hongshu/Hongshu2-32B-chat-v1_1-240419-lora-hql-v1"
lora_adapter=""
if [ -n "$loras" ]; then
	    lora_adapter="--adapters ${loras} --backend pytorch"
fi
...
lmdeploy serve api_server ${model_path} --server-port 3335 --model-name ${model_name} --tp ${tp} --chat-template qwen2.json --cache-max-entry-count ${pct} --session-len ${context_len} ${lora_adapter}
#--quant-policy 8
