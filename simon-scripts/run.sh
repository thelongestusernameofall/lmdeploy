...
lmdeploy serve api_server ${model_path} --server-port ${port} --model-name ${model_name} --tp ${tp} --cache-max-entry-count ${pct} 

#lmdeploy serve gradio ${model_path} --server-port 3333 --model-name ${model_name} --tp ${tp} --chat-template llama3.json
#--quant-policy 8
