...
qos_config="--qos-config-path qos_config.json"
...
lmdeploy serve api_server ${model_path} --server-port 3335 --model-name ${model_name} --tp ${tp} --chat-template qwen2.json --cache-max-entry-count ${pct} --session-len ${context_len} ${qos_config}
#--quant-policy 8
